...
function cleanZncDuplicates(text: string): string {
	...
	// Pattern: bouncername BADGE realname BADGE hostmask
	const zncPattern =
		/^(\w+)\s+(VIP|Mod|Admin|SysOp|SYSOP|Developer|Dev)\s+(\w+)\s+(VIP|Mod|Admin|SysOp|SYSOP|Developer|Dev)/i;
	const match = text.match(zncPattern);

	if (match) {
		// Extract the real username (third capture group) and the rest
		const realUsername = match[3];
		const restOfText = text.substring(match[0].length);
		return realUsername + restOfText;
	}
...
function parse(
	...
	// NEW: Clean ZNC playback duplicates FIRST before any other parsing
	let processedText = text;

	// Detect if this is likely ZNC playback from *buffextras
	if (
		message?.from?.nick === "*buffextras" ||
		text.match(/^\w+\s+(VIP|Mod|Admin)\s+\w+\s+(VIP|Mod|Admin)/i)
	) {
